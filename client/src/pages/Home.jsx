...
const Home = () => {
  return (
    <main className="relative overflow-hidden section-hero transition-colors duration-300">
      <div className="pointer-events-none absolute inset-0 opacity-30 dark:opacity-80 [mask-image:radial-gradient(circle_at_center,black,transparent)] transition-opacity duration-300">
      ...
      <section className="relative mx-auto flex min-h-[70vh] max-w-6xl flex-col justify-center gap-10 px-4 py-16 text-center lg:flex-row lg:items-center lg:text-left">
        <div className="flex-1 space-y-6">
          <p className="inline-flex items-center gap-2 rounded-full border border-white/20 dark:border-white/10 px-4 py-2 text-xs uppercase tracking-[0.3em] text-white/80 dark:text-white/70 transition-all duration-300">
            Buy & sell used items
          </p>

          <h1 className="font-display text-4xl leading-tight text-white sm:text-5xl lg:text-6xl transition-colors duration-300">
          ...
          <p className="text-lg text-white/90 md:text-xl transition-colors duration-300">
        ...
        <div className="flex flex-col gap-6 text-center lg:text-left">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {features.map((feature) => (
              <div key={feature.title} className="card-light text-left">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white transition-colors duration-300">
                  {feature.title}
                </h3>
                <p className="mt-3 text-sm text-gray-600 dark:text-white/70 transition-colors duration-300">{feature.text}</p>
              </div>
            ))}
          </div>
        </div>
